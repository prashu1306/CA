...
def train():
    ...
    lambda2 = lambda epoch : (1 + epoch)**(-0.4)  
...
f = open("plotting_ppoca.txt", "a")
f1 = open("plotting_ppoca_sdt.txt", "a")
...
for i in range(0,n_seed):
      print('i =', i)
      indices , reward_list_ppo_ca[i],compute_time[i] = PPO_critic_actor(seed[i][0])
      #print(i)
...
reward_ppo_ca = np.mean(reward_list_ppo_ca, axis = 0)
compute_t = np.mean(compute_time , axis = 0)

for i in reward_ppo_ca:
    f.write(str(i)  + '\n')
f.close()        
...
print('compute_time=',compute_t[0])
